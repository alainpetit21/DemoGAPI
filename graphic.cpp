...
/*==============================================================================================================*/
int
VID_ApplyScisor(IMAGE* pDes, int& nDx, int& nDy, int& nDw, int& nDh, int& nSx, int& nSy, int& nSw, int& nSh) 
{
  int ret= 0;                                          
...
VID_FillRect_i(IMAGE* pDes, int left, int top, int right, int bottom, ushort lColor)
{
  ...
  if(!(VID_ApplyScisor(pDes, left, top, w, h, left, top, w, h))) return;
...
{
  if(!(VID_ApplyScisor(pDes, nDx, nDy, nDhw, nDhh, nSx, nSy, nSw, nSh))) return;
...
/*==============================================================================================================*/
void
VID_LoadRAW565(IMAGE **ppBitmap, ushort **bufPixel, cstring szBitmap)
{
	PTFILE fichierR;
	RAW565_HDR	rawHeader;
	int w,h;

	fichierR= FS_CreateFile((string)szBitmap, OPT_READ);
	FS_ReadFile(fichierR, &rawHeader, sizeof(RAW565_HDR));
	w= rawHeader.w;
	h= rawHeader.h;
	...
	(*ppBitmap)->h= h;
	(*ppBitmap)->pData= (*bufPixel);
	FS_ReadFile(fichierR, (*ppBitmap)->pData, w*h*2);
	FS_CloseFile(fichierR);
	(*ppBitmap)++;
	(*bufPixel)+= w*h;
}
